package com.mengc.nettySpring.netty.netty.busAtomHandler;

import com.mengc.nettySpring.netty.bean.ResultBean;
import com.mengc.nettySpring.netty.bean.UploadFile;
import com.mengc.nettySpring.netty.util.FileUtil;
import com.mengc.nettySpring.netty.util.PropUtil;
...
public class FileByteServerHandler  extends ChannelInboundHandlerAdapter {
    public FileByteServerHandler(ArrayList<byte[]> array){
        super();
        this.array=array;
    }
    public FileByteServerHandler(){
        super();
    }
    private ArrayList<byte[]> array= new ArrayList<byte[]>();
    private String baseUrl = PropUtil.getV("baseUrl");
...
//                    System.out.println(new String (file_content));
                    String file_dir = this.baseUrl+File.separator+uploadFileBean.getDir();
                    FileUtil.createDir(file_dir);
                    FileOutputStream fos = new FileOutputStream(file_dir+File.separator+uploadFileBean.getFileName());
...
//            ctx.pipeline().remove("fileByteServerHandler");
            ctx.fireChannelRead(msg);
        }
    }

    private void testBake(ChannelHandlerContext ctx, Object msg)throws IOException, ClassNotFoundException {
        try {
            byte[] b = this.byteMergerAll(this.array);
            String endflag = new String(b);
